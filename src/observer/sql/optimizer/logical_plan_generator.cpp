...
RC LogicalPlanGenerator::create_plan(FilterStmt *filter_stmt, unique_ptr<LogicalOperator> &logical_operator)
{
  ...
  RC rc = create_expression(filter_stmt, expr);
  if(RC::SUCCESS != rc){
    return rc;
  }
  if(expr){
    predicate_oper = unique_ptr<PredicateLogicalOperator>(new PredicateLogicalOperator(std::move(expr)));
  }
...
RC LogicalPlanGenerator::create_plan(UpdateStmt *update_stmt, unique_ptr<LogicalOperator> &logical_operator)
{
  ...
  unique_ptr<UpdateLogicalOperator> update_oper(new UpdateLogicalOperator(table, field_name, values));
